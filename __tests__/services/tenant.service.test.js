const { isValidTenant, isValidTenantOrigin } = require('../../src/services/tenant.service')

describe('Tenant Functions', () => {
    
    describe('isValidTenant', () => {
        test('should return true for a valid tenant', () => {
            expect(isValidTenant('vmax')).toBe(true);
        });

        test('should return true for another valid tenant', () => {
            expect(isValidTenant('fit_dad')).toBe(true);
        });
    ...
    describe('isValidTenantOrigin', () => {
        ...
        test('should return false for a valid tenant and invalid origin', () => {
            expect(isValidTenantOrigin('vmax', 'http://invalid-origin.com')).toBe(false);
        });

        test('should return false for an invalid tenant and any origin', () => {
            expect(isValidTenantOrigin('non_existent_tenant', 'http://localhost:4200')).toBe(false);
        });

        test('should return false for a valid tenant but not allowed origin', () => {
            expect(isValidTenantOrigin('fit_dad', 'http://another-origin.com')).toBe(false);
        });

        test('should return true for a valid tenant and allowed origin', () => {
            expect(isValidTenantOrigin('fit_dad', 'http://localhost:4200')).toBe(true);
        });
    });
});
